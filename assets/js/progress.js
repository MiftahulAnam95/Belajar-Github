window.GithubLabProgress = (() => {
  const key = "github-beginner-lab-progress-v1";
  ...
  const saveState = (state) => {
    ...
    window.dispatchEvent(new CustomEvent("github-progress-updated", { detail: nextState }));
    return nextState;
  };
  ...
  const reset = () => {
    ...
    applyTheme();
    window.dispatchEvent(new CustomEvent("github-progress-updated", { detail: getState() }));
  };
  ...
  const calculateProgress = (data = window.GithubLabData) => {
    const state = getState();
  ...
  const getBadges = (data = window.GithubLabData) => {
    const state = getState();
    return data.badges.map((badge) => ({ ...badge, unlocked: badge.check(state) }));
  };

  const getRecommendation = (data = window.GithubLabData) => {
    const state = getState();
