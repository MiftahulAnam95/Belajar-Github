  ...
  const renderNav = () => {
    ...
    target.innerHTML = `
      <nav class="navbar navbar-expand-lg sticky-top app-navbar">
        <div class="container">
          <a class="navbar-brand" href="${base}index.html" aria-label="GitHub Beginner Lab">
            <span class="brand-mark"><i class="bi bi-github"></i></span>
            <span class="brand-text"><strong>GitHub Lab</strong><span>Beginner</span></span>
          </a>
    ...
    themeToggle?.addEventListener("click", () => {
      const state = window.GithubLabProgress.getState();
      window.GithubLabProgress.setDarkMode(!state.darkMode);
      showToast(!state.darkMode ? "Mode gelap aktif" : "Mode terang aktif");
    });

    window.GithubLabProgress.applyTheme();
  };
