(() => {
  ...
  const progress = window.GithubLabProgress;
  ...
  const toast = (message) => window.GithubLabLayout?.showToast(message);
  ...
  const recallStorageKey = "github-beginner-lab-recall-answers-v1";
...
<html lang="id">
  ...
  const renderLessonGrid = (activeId = data.lessons[0]?.id) => {
  ...
  const renderLessonDetail = (lessonId = data.lessons[0]?.id) => {
  ...
  const renderLessonsPage = () => {
    ...
    if (data.lessons.some((lesson) => lesson.id === hashId)) {
      location.replace(lessonUrl(hashId));
      return;
    }
  ...
  const renderDebugPage = () => {
    ...
    const render = (id = active) => {
        ...
        <div class="challenge-body">
          ...
          <div class="debug-editor-grid">
            <div class="debug-editor-wrap">
              <label class="editor-title" for="debugCode"><i class="bi bi-terminal"></i> editor command perbaikan</label>
      ...
      const renderPreview = () => {
        saveCurrent();
        previewFrame.srcdoc = buildPreviewDocument("Draft perbaikan GitHub", codeInput.value);
      };
  ...
  const renderProgressPage = () => {
    ...
    target.innerHTML = `
      ...
      <div class="lesson-actions">
        <a class="btn btn-primary" href="materi.html"><i class="bi bi-arrow-right"></i> Lanjut materi</a>
